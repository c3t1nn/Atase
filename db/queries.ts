...
export const getUnitWords = cache(async (unitId: number) => {
  ...
  if (!data) return { title: "", words: [] };

  const rawWords = data.lessons.flatMap((lesson) => 
    ...
    const isSingleWordQuestion = !word.question.trim().includes(" ");
    const isSingleWordAnswer = !word.answer.trim().includes(" ");
    
    return (
      word.answer !== "" && 
      isSingleWordQuestion && 
      isSingleWordAnswer &&
      !word.question.includes("?")
    );
  });
